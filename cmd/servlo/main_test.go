...
// A worktree that pins its own PHP version in .servlo.yaml must be served by that
...
func TestScanWorktrees_vhostHonoursTheWorktreePHPPin(t *testing.T) {
	...
	if !strings.Contains(string(vhost), "servlo-php83-fpm") {
		t.Errorf("worktree vhost must route to the pinned 8.3 container, got:\n%s", vhost)
	}
	if strings.Contains(string(vhost), "servlo-php84-fpm") {
