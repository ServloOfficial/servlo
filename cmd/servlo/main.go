...
// registerCommands hangs every subcommand off root.
//
// Split out of main so a test can walk the real command tree. What a
// message tells an operator to run is only useful if it names a command
// that exists, and `servlo service preset install` did not: five error
// messages pointed at it, each one firing at the moment something was
// already wrong. Nothing checked, because nothing could reach the tree.
