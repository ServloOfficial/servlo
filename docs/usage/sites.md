...
A site is a directory on this server, served on a real domain, with a PHP
version and a framework of its own.

## Four ways to add one

| | How | Where |
|---|---|---|
| **Clone from Git** | Servlo mints an ed25519 deploy key for you to paste into the repository's deploy keys, verifies the connection, then clones | Dashboard |
| **Upload a ZIP** | Uploaded, extracted, document root detected | Dashboard |
| **Point at a folder** | A directory already on the server | Dashboard, or `servlo link example.com` |
| **Install an app** | A fresh WordPress and the other apps in the store, with the database created and the admin account set up | Dashboard, or `servlo apps install` |

A fifth route exists for a site that is already running somewhere else:
`servlo import site <directory> <domain> --dump dump.sql` takes what you
actually have when moving off shared hosting — a directory of files and a `.sql`
dump — works out the framework and document root, registers the site, writes the
vhost and loads the dump into a database of its own. It does not copy the files.
See [Importing a site](/usage/import).

In every case Servlo asks for the domain, PHP version and framework, creates the
vhost and registers the site, and the **Get SSL** button appears immediately with
its DNS check already running.

::: tip One deploy key per site, not per server
The key is generated here rather than pasted in, because the alternative is a
form asking you to paste a private key into a browser. It is scoped to a single
repository and belongs to a single site, so a site that gets compromised does not
hand over an account that can read every repository you can, and revoking one
site's access is deleting one key rather than working out what else breaks. The
private half never leaves the server or appears in an API response.
:::
...
| `servlo park [dir]` | Register every PHP project inside `dir` as a site, and keep doing so as new ones appear (defaults to cwd). Each is served on the domain its own directory is named for |
| `servlo unpark [dir]` | Remove a parked directory and unlink all its sites |
| `servlo link [domain]` | Register the current directory as a site. The argument is the full domain, e.g. `example.com`; without it the directory name is used when it is already a domain, or the `.servlo.yaml` domains are. On a fresh project in an interactive terminal it runs the `servlo init` wizard first |
...
A site is served on a real domain, and can answer to as many as you give it.
Naming, aliases, the canonical www form, redirects and what happens when two
sites claim the same name are all on their own page: [Domains](/usage/domains).
...
A staging site loses its password file too. It is named for the domain and kept at 0644 so nginx's workers can read it, and nothing took it away when the site went, so every staging site ever removed left its hash on disk. It holds a bcrypt hash rather than a password and no vhost names it once the site is gone, so this is a leftover rather than a way in, but it is a credential and it has no reason to outlive what it guarded.

Its PHP settings go as well: the whole `php/sites/<handle>` directory, holding the ini servlo manages, the one you wrote by hand, and the backups of yours. Both are mounted into the site's runtime by name, so leaving them would put the next site of that handle on the last one's php.ini.

A parked site keeps its certificate, its override, its backup schedule, its password file and its PHP settings, because unlinking one is a tombstone rather than a removal: linking the directory again brings the site back and it has to come back with them. Most of what unlinking takes is a unit or a generated file that relinking rewrites for nothing. These are not: a certificate costs a rate-limited exchange with an authority that can refuse, an override is something a person typed, and a parked staging site's vhost still names its password file, so removing it would leave a relinked site answering 500 to everybody rather than asking for a password.
