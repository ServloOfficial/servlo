...
Framework workers (queue, schedule, horizon, reverb, custom) run as systemd user units. systemd restarts a worker that crashes, but it also stops trying after enough rapid failures and marks the unit `failed`. Once a unit is in `failed` state, systemd will not lift it out without an explicit reset.

`servlo worker heal` is the recovery primitive: it reset-clears the failed state and starts the unit again, on every surface (CLI, dashboard, TUI).
...
Heals one worker for the site at the current working directory. Equivalent to `systemctl --user reset-failed servlo-queue-<site> && systemctl --user start servlo-queue-<site>` but runs through the same code path the dashboard uses.
...
- **It does not write `.servlo.yaml`.** A failed worker is a transient runtime condition, not a change of user intent. Adding or removing workers from a site's enabled list still belongs to `servlo worker add` / `servlo worker remove` (or the equivalent dashboard / TUI actions).
...
- **It does not touch a site's other units.** A site's own containers (`servlo-fp-`, `servlo-custom-`, `servlo-cfpm-`) and its backup timers (`servlo-backup-`, `servlo-backup-verify-`) are named after the site the same way a worker is, but they are not workers and heal skips them. A backup that fails raises its own [alert](/usage/alerts) rather than appearing here, and starting it from a heal button would take an unscheduled backup at whatever time you pressed it.
