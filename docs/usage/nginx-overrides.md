...
A block servlo declares is never retired either, whatever its name. Commenting out the line that opens one would leave its body and its closing brace behind as loose `http {}` statements, and nginx refuses the whole file rather than the one directive. The only block servlo ships at this level is the websocket upgrade `map` every proxying vhost reads `$connection_upgrade` from, and a `map` of your own sits alongside it: nginx allows as many as you like so long as they set different variables.
