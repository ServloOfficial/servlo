...
Servlo renews a secured site's certificate on its own before it lapses. The watcher sweeps every secured site twice a day, and again the moment it starts, so a machine that was off for a month looks at its certificates as soon as it comes back rather than half a day later. A certificate within roughly 30 days of its `NotAfter`, or already expired, missing or corrupted, is reissued in place; one comfortably clear of that window is left untouched, so the check is cheap and silent. When anything was reissued, nginx is reloaded once at the end of the sweep, because nginx serves the certificate it loaded at its last reload and a renewal nobody reloads for is a new file on disk with the old certificate still on the wire. `servlo status` continues to surface the same 30-day expiry warning under `[TLS Certificates]`, but you no longer need to act on it manually; a long-lived site that just keeps running self-heals its own certificate.
