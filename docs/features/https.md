...
> [!NOTE]
> HTTPS is only available when servlo is managing DNS. If you installed in disabled-DNS mode (`dns.enabled: false`, sites under `*.localhost`), the mkcert root CA was never installed, the `servlo init` wizard skips the "Enable HTTPS?" question, the per-site HTTPS toggle in the dashboard becomes a muted lock icon, and `servlo secure` refuses with `HTTPS requires servlo-managed DNS, set dns.enabled: true and re-run servlo install`. A project whose committed `.servlo.yaml` carries `secured: true` is linked over http on a disabled-DNS install rather than registered as a non-functional HTTPS site. See DNS for the toggle and how to switch back.
...
If you would rather not install a package at all, run `servlo dns:disable` to serve your sites over plain http on `*.localhost`, which needs no certificate.
