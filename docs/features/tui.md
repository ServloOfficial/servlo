...
`servlo tui` opens a btop-inspired full-screen dashboard in your terminal. It shows sites, services, workers, and logs in one glance, updates live, and lets you drive most of the same operations the web UI exposes without ever leaving the terminal.
...
A clickable tab strip sits at the top and switches the whole screen between three views: **Dashboard**, **Sites**, and **Services**. The TUI opens on the **Dashboard**. Click a tab to switch, or cycle with `ctrl+←` / `ctrl+→` from the keyboard. The active tab reads as a filled accent pill in the servlo palette; the others sit dim.
...
- **Services pane (Services tab, left column)** is a compact list of built-in services (mysql, redis, postgres, meilisearch, rustfs), custom services, and every site-owned worker (`queue-<site>`, `schedule-<site>`, `horizon-<site>`, `reverb-<site>`, and custom framework workers). Each row shows a running dot, how many sites use it, and `pinned` / `custom` tags where applicable.
...
| `e` | Edit / rename the focused domain (opens inline input prefilled with the short name; commit runs `servlo domain add <new>` then `servlo domain remove <old>` as a sequence) |
...
| Key | Action |
| --- | --- |
| `:` | Open the command palette, type any `servlo <args>` (e.g. `service restart redis`) and press enter to shell out exactly as if you'd typed it in a regular terminal |
...
- **FPM / custom container**: `podman logs -f servlo-php<ver>-fpm` for PHP sites, or `servlo-custom-<name>` for custom container sites.
- **Workers**: `journalctl --user -u servlo-queue-<site>` (and the same for schedule, reverb, horizon, custom framework workers). Workers are systemd user units, not containers, so their output lives in the user journal.
...
Press `Y` (on the Sites tab) to swap the detail pane for the System overview, the terminal-side counterpart to the web UI's System tab. Sections cover every shared subject servlo manages outside of an individual site, with informational rows for status and reversible toggles for safe operations:
...
The TUI draws state from the same sources `servlo-ui` uses, in-process:

- Subscribes to the shared eventbus so any mutation the TUI itself triggers shows up immediately (150 ms debounce).
- Re-queries every 2 seconds as a safety net, so changes made from another terminal (`servlo service stop redis` in a different shell) surface within a couple of seconds.
...
- **Non-interactive shells**: `servlo tui` exits with an error when stdout isn't a TTY (piped output, CI). Run it inside a real terminal.
- **Worker log says nothing**: check the worker is actually running (`servlo status` or the Workers section of the detail pane). Journal logs only exist while the unit has run at least once.
