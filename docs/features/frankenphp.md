...
Servlo can serve a PHP site through a per-site [FrankenPHP](https://frankenphp.dev/) container instead of the shared PHP-FPM container. FrankenPHP keeps PHP resident in memory, serves HTTP directly, and supports a worker mode that reuses a single PHP process across requests.
...
**From `.servlo.yaml`** (commits the choice to the repo, so everyone who links the project gets the same runtime):
...
Flip back to FPM with `servlo runtime fpm`. `servlo runtime` without an argument prints the current runtime. Both surfaces restart the container, regenerate the nginx vhost, and reload nginx automatically.

If a site loses its `runtime: frankenphp` line in `.servlo.yaml` (a git revert, a branch switch, a manual edit) and you re-link it, servlo reconciles the change for you: the leftover `servlo-fp-<site>.container` quadlet and its container are removed during the link, so a stale FrankenPHP container is never left running outside servlo's start/stop control.

---

## What happens under the hood

For a FrankenPHP site, servlo:
...
2. Writes a per-site quadlet `servlo-fp-<site>.container` that mounts the project at its host path and runs the framework's entrypoint.
3. Generates an nginx vhost that reverse-proxies to `servlo-fp-<site>:8000`.
4. Starts the container, reloads nginx.

The container joins the shared `servlo` Podman network, so services like `servlo-mysql`, `servlo-redis`, and `servlo-meilisearch` are reachable by hostname.

Pause semantics mirror FPM: `servlo pause <site>` (or the dashboard pause toggle, or `site_control action=pause`) stops `servlo-fp-<site>` alongside the paused-vhost swap, and `servlo unpause <site>` starts it again. The container is only running while the site is active, so a long-idle FrankenPHP site doesn't keep a process resident.
...
- **Non-worker (`runtime_worker: false`, default)**: servlo runs `frankenphp php-server -r public/`. Each request boots Laravel from scratch; code edits take effect on the next request, same as FPM. You still get FrankenPHP's HTTP/2, HTTP/3, and TLS, but not Octane's per-request speedup.
- **Worker (`runtime_worker: true`)**: servlo runs `php artisan octane:start --server=frankenphp --host=0.0.0.0 --port=8000 --workers=auto`. Octane keeps Laravel resident; requests skip the full bootstrap. Octane registers Symfony Console signal handlers which need the `pcntl` PHP extension; it is baked into servlo's derived FrankenPHP image (see the Extensions section) so the container boots straight into Octane.
...
In worker mode servlo also passes `--watch`, which reloads the resident worker on any `.php`, `.env`, `.yaml`, or `.twig` change, so file edits take effect without a manual restart. `runtime/frankenphp-symfony` (optional) plugs Symfony's Runtime into the same worker loop for slightly lower per-request overhead.
...
To override the defaults for a specific project, add a user framework overlay at `~/.config/servlo/frameworks/<name>.yaml` with a `frankenphp:` block, or commit a full framework definition alongside the project.

---

## Workers

Queue workers, schedulers, Reverb, Horizon, and any framework-defined worker continue to work unchanged: servlo spawns each as its own systemd service and `podman exec`s into the FrankenPHP container for the site. Laravel `queue:work` and Symfony `messenger:consume` both run alongside the web worker without conflict.
...
- **SPX profiler** (`servlo profile`). SPX profiles per request and does not hook Octane's resident-worker loop (its `/_spx` UI 404s under Octane, and servlo's profiler injection is fastcgi-only), so it stays FPM-only. The global profiler toggle still profiles your FPM sites; to profile a FrankenPHP site with SPX, switch it back to FPM (`servlo runtime fpm`, run from the project).

Everything else, the full runtime extension set, Xdebug, the dump()/dd() bridge, the Debug window (`servlo_devtools`), per-site php.ini, bun, and Pest browser testing, is supported.
...
The Web UI site detail panel shows an orange **FrankenPHP** badge next to the framework and services, with a `worker` suffix when worker mode is on. The same badge appears in `servlo tui` beside the PHP version line.
