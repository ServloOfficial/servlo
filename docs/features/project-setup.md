...
Commands that operate on the current site (`servlo open`, `servlo runtime`, `servlo worker`, `servlo stripe`, `servlo env`, `servlo domain`, `servlo share`, and the worker-backed `servlo reverb`, `servlo schedule`, `servlo queue`) need the directory to be linked. When you run one in a project that has not been linked yet, servlo offers to link it for you instead of stopping with an error:
