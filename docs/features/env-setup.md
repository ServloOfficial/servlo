...
Services prefixed with `From .servlo.yaml` were not referenced in the env file but are listed in `.servlo.yaml`. Their connection values are written and the service is started so the project is ready to use them.

---

## Automatic backup

The first time `servlo env` modifies an existing `.env` that has not yet been touched by servlo, it saves a copy as `.env.before_servlo` in the project root and adds the file to `.gitignore` (if one exists). Once servlo has written its connection values to `.env`, subsequent runs skip the backup entirely, so deleting `.env.before_servlo` is safe and it will never be recreated.

The backup lets you:
- **See exactly what servlo changed**: diff `.env.before_servlo` against `.env`
- **Restore the original** with `servlo env:restore` (see below)
- **Preserve original Sail credentials**: `servlo import sail` reads S3 configuration from `.env.before_servlo` when it exists, so the original Sail MinIO bucket and endpoint are used even after servlo has overwritten `.env`

Example output when the backup is created:

```
Updating existing .env...
  Backed up original .env to .env.before_servlo
  Detected mysql:        applying servlo connection values
  ...
Done.
```
...
Copies `.env.before_servlo` back to `.env`. Useful when switching a project back to Laravel Sail or another local environment.

After restoring, run `servlo env` again to re-apply servlo connection values.

---

## Personal overrides (`.env.servlo_override`)

`servlo env` writes servlo's own defaults into `.env` (for example `DB_USERNAME=servlo`). When you want different values on your machine without hand-editing `.env` after every run, drop them into a personal `.env.servlo_override` file in the project root. It is plain dotenv syntax, it is never committed (servlo adds it to `.gitignore`), and every `KEY=VALUE` in it is layered on **last** when you run `servlo env`, winning over servlo's defaults and every computed value (`DB_DATABASE`, `APP_URL`, and so on).
...
`servlo env:override` creates the file from a commented template (and gitignores it) on first run, and seeds any `KEY=VALUE` arguments you pass. Run it with no arguments to just scaffold the file, then edit it by hand. The file is the source of truth, so you can also create or edit `.env.servlo_override` directly (or in the dashboard's Env tab); whenever it is present, `servlo env` keeps it listed in `.gitignore`. Values are written into `.env` verbatim, so quote any value that needs it (spaces, `#`), e.g. `DB_PASSWORD="p@ss word"`. Because it is a partial overlay rather than a full env file, `servlo env:check` skips it.

### Using your own service instead of servlo's container

The one reserved key, `SERVLO_EXTERNAL_SERVICES`, lists services servlo should treat as externally managed (you run your own instance). For each named service servlo still writes its connection variables into `.env`, but it does **not** start the container and does **not** create the project database or S3 bucket. Combine it with the connection overrides that point at your own instance:

```dotenv
# .env.servlo_override: point this project at a host (system) database
...
Use `host.containers.internal` for the host rather than `127.0.0.1`. The override is read inside the PHP-FPM container, where `127.0.0.1` is the container's own loopback, not your machine. servlo keeps a `host.containers.internal` entry in the container that resolves to the host, so a containerized app reaches a host MySQL, MariaDB or Postgres over it with no extra setup. For MySQL use `DB_PORT=3306`.

The value is comma or space separated, so `SERVLO_EXTERNAL_SERVICES=postgres, redis` opts both out. The reserved key is consumed by servlo and is never written into `.env`.

The variables servlo writes for an external service are the ones your framework reads, since they are what the override file then overrides. An external MariaDB on Symfony gets the `DATABASE_URL` Doctrine reads, not the Laravel-shaped `DB_*` keys Symfony has no use for. A framework whose env file is a returned PHP array, as Magento's `app/etc/env.php` is, cannot take an override at all: its keys are dotted paths and `.env.servlo_override` is dotenv, so there is no key servlo could write that you would then point at your own instance. servlo writes nothing for it, tells you so, and leaves the connection in `env.php` to you.
...
Running `servlo env` on a project that already has a `.env` is safe; it only updates connection-related keys and leaves everything else untouched.

---

## Checking for drift

`servlo env:check` compares all `.env` files in the project against `.env.example` and shows a single table of any keys that are out of sync:
