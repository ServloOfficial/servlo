...
- **Servlo**: current version, "Up to date" or a yellow "update available" banner with an **Open terminal & update** button, Autostart and LAN status pills, plus **Check for updates** and **Manage →** in the footer.
...
- **Follow in terminal**: the terminal icon in a log pane's header opens the host's terminal emulator tailing the same unit (`podman logs -f`, `tail -f`, or `journalctl -f` depending on the platform and the unit), so a long tail can outlive the browser tab. It uses `$TERMINAL` when set. Remote dashboards show the same action once `servlo remote-control full-access on` is set; the terminal opens on the host that runs Servlo.
...
Shows the current version. When an update is available, the Servlo entry exposes an **Open terminal & update** button that launches the host's terminal emulator running `servlo update`. A remote dashboard shows the same action once `servlo remote-control full-access on` is set; the terminal opens on the host that runs Servlo.
