...
The persistent half asks which file decides the value, not whether servlo's own drop-in says the right thing. `systemd-sysctl` reads every `*.conf` across `/etc/sysctl.d`, `/run/sysctl.d` and `/usr/lib/sysctl.d`, sorts them by filename regardless of which directory they came from, and lets the lexicographically latest one win; a file of the same name in an earlier directory masks the later one entirely. Servlo's is `99-servlo-ports.conf`, and Ubuntu ships `/etc/sysctl.d/99-sysctl.conf` as a symlink to `/etc/sysctl.conf`, which sorts after it. So the most obvious file to edit, and the one a hardening guide names, decides this setting rather than servlo's, and doctor says which file is deciding it and what that file says.
