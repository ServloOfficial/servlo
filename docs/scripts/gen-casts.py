...
# ---- mcp: a terminal Claude Code session driving servlo (16 content lines) ----
m = Cast(88, 17)
m.type('~/code $ ', 'claude')
m.line(col('dim', '╭─ Claude Code ───────────────── servlo-mcp connected ─╮'), 0.5)
m.line(col('dim', '╰────────────────────────────────────────────────────╯'), 0.2)
m.line('')
...
m.line('  ' + col('dim', '⎿ ') + col('vio', 'site.link') + col('dim', ' (servlo) … ') + col('ok', 'done'), 0.9)
m.line('  Created ' + col('url', '~/code/billing') + ', detected Laravel 11, set PHP ' + col('bold', '8.4') + ', provisioned TLS.', 0.5)
m.line('  ' + col('dim', '⎿ ') + col('vio', 'service.start redis') + col('dim', ' (servlo) … ') + col('ok', 'done'), 0.9)
...
m.line('')
m.line(col('amb', '● ') + col('dim', '⎿ ') + col('vio', 'exec.artisan migrate') + col('dim', ' (servlo) … ') + col('ok', 'done'), 0.9)
