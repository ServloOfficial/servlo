...
c.line(col('dim', '→ provisioning TLS… ') + col('ok', '✓ issued'))
c.line(col('ok', '✓ linked') + col('dim', ' in 1.8s'))
c.line('')
c.line('  ' + col('dim', 'Site') + '   ' + col('url', 'https://acme.example.com'))
...
s2.line(col('dim', '→ TLS · ') + col('ok', '✓ issued'))
s2.line(col('ok', '✓ live at ') + col('url', 'https://acme.example.com'))
s2.wait(3.0)
