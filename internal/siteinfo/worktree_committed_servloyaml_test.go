...
// TestWorktreeWithCommittedServloYaml_drivesEnrichedVersions exercises the
// full read path that the dashboard relies on: a worktree's checkout has a
// .servlo.yaml that we did NOT write through the servlo UI (think: the user's
...
//
// Writing the .servlo.yaml here as raw YAML (instead of via SaveProjectConfig)
// matches what `git checkout` puts on disk byte-for-byte and proves the read
// path doesn't depend on servlo having authored the file.
func TestWorktreeWithCommittedServloYaml_drivesEnrichedVersions(t *testing.T) {
	main, checkout := makeWorktreeFixture(t, "feat-php84")

	committedYAML := []byte(`# This file is the project's .servlo.yaml as committed in git.
php_version: "8.4"
node_version: "24"
`)
	if err := os.WriteFile(filepath.Join(checkout, ".servlo.yaml"), committedYAML, 0644); err != nil {
		t.Fatalf("write committed .servlo.yaml: %v", err)
	}
	...
	if w.PHPVersion != "8.4" {
		t.Errorf("PHPVersion = %q, want %q (from committed .servlo.yaml)", w.PHPVersion, "8.4")
	}
	if !w.PHPVersionOverride {
		t.Errorf("PHPVersionOverride = false, want true so the dashboard does not show the inherited badge")
	}
	if w.NodeVersion != "24" {
		t.Errorf("NodeVersion = %q, want %q (from committed .servlo.yaml)", w.NodeVersion, "24")
	}
	...
	got, err := os.ReadFile(filepath.Join(checkout, ".servlo.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if unmarshalled["php_version"] != "8.4" || unmarshalled["node_version"] != "24" {
		t.Errorf("on-disk .servlo.yaml mutated: %v", unmarshalled)
	}
}
