...
// HostProxyGate is the decision for whether servlo may supervise a dev-server
...
func HostProxyGate(command string, disabled, skipConfirm, approved, canPrompt bool) (proceed, ask bool, reason string) {
	if command == "" {
		// Proxy-only: servlo supervises nothing, so neither the disable switch
	...
	if !canPrompt {
		return false, false, "command not approved; re-run `servlo link` interactively, pass --yes, or set host_proxy.skip_confirmation: true"
	}
	return false, true, ""
}
