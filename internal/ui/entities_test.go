...
func TestDsnDatabase(t *testing.T) {
	cases := []struct {
		...
		{"mongodb://root:servlo@servlo-mongo:27017/astrolov?authSource=admin", "mongo", "astrolov"},
		{"mongodb://root:servlo@servlo-mongo:27017/?authSource=admin", "mongo", ""},
		{"mysql://root:servlo@servlo-mysql:3306/shop", "mysql", "shop"},
		// Another service's DSN must not claim this engine's databases.
		{"mongodb://root:servlo@servlo-mongo:27017/astrolov", "mysql", ""},
		// An alternate instance is not the canonical one.
		{"mongodb://root:servlo@servlo-mongo-6:27017/astrolov", "mongo", ""},
		{"not a url at all", "mongo", ""},
		{"redis://servlo-redis:6379", "redis", ""},
		// A multi-segment path is not a database name.
		{"http://servlo-rustfs:9000/bucket/key", "rustfs", ""},
	}
