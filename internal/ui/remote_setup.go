...
//     by `servlo remote-setup token`, and that token must not be expired. The
...
func handleRemoteSetup(w http.ResponseWriter, r *http.Request) {
	...
	if !subtleEqual(token.Token, code) {
		...
		if closed {
			http.Error(w, fmt.Sprintf("invalid code — too many wrong attempts, code revoked. Generate a new one with `servlo remote-setup`."), http.StatusUnauthorized)
			return
		}
...
func mkcertCAROOT() (string, error) {
	...
	if _, err := os.Stat(bin); err != nil {
		return "", fmt.Errorf("mkcert binary not found at %s — run `servlo install` first", bin)
	}
...
# servlo remote setup — generated by /api/remote-setup
# Sets up the calling machine to resolve *.%[2]s against this servlo server
...
# ── 2. install the servlo root CA ──────────────────────────────────────────
# Use a dedicated CAROOT directory just for the remote servlo CA so we never
...
# doesn't match parent's PublicKey" — breaking 'servlo secure' on any host
# that had ever run this script.
step "Installing the servlo remote root CA into the system trust store"
SERVLO_REMOTE_CAROOT="$HOME/.local/share/servlo-remote-ca"
mkdir -p "$SERVLO_REMOTE_CAROOT"
echo "$CA_B64" | base64 --decode > "$SERVLO_REMOTE_CAROOT/rootCA.pem"
CAROOT="$SERVLO_REMOTE_CAROOT" mkcert -install
ok "servlo remote root CA trusted (isolated from any local mkcert CA)"
...
case "$OS_KIND" in
  linux)
    ...
    DNSMASQ_DROPIN="/etc/dnsmasq.d/servlo.conf"
    RESOLVED_DROPIN="/etc/systemd/resolved.conf.d/servlo-test.conf"
    ...
    if [ "$nm_uses_dnsmasq" = "1" ]; then
      ...
      echo "$LINE" | sudo tee "$NM_DNSMASQ_DIR/servlo.conf" >/dev/null
      sudo systemctl restart NetworkManager
      ok "Wrote $NM_DNSMASQ_DIR/servlo.conf and restarted NetworkManager"
      ...
      # that forwards .test queries to the servlo server. Requires systemd
...
Setup complete. Open the servlo dashboard from this device at:
  http://$SERVER_IP:7073
...
⚠  Server IP is hard-coded to $SERVER_IP in the resolver dropin.
   If the servlo server later moves to a different network and gets a
   new LAN IP, .$TLD lookups from this machine will start failing.
   To fix it, run 'servlo remote-setup' on the server again, then re-run
   the new curl one-liner from this machine — it overwrites the dropin
   in place. No need to revert anything first.

To revert later:
  • Linux NM:        sudo rm /etc/NetworkManager/dnsmasq.d/servlo.conf && sudo systemctl restart NetworkManager
  • Linux dnsmasq:   sudo rm /etc/dnsmasq.d/servlo.conf && sudo systemctl restart dnsmasq
  • Linux resolved: sudo rm /etc/systemd/resolved.conf.d/servlo-test.conf && sudo systemctl restart systemd-resolved
  • macOS:           sudo rm /etc/resolver/$TLD
  • Cert:            CAROOT="$SERVLO_REMOTE_CAROOT" mkcert -uninstall && rm -rf "$SERVLO_REMOTE_CAROOT"
EOF
`
