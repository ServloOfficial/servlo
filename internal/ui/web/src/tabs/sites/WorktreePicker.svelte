...
<div bind:this={pickerEl} class="relative inline-flex shrink-0">
  {#if hasWorktrees}
    <button
      type="button"
      onclick={() => (open = !open)}
      class="flex items-center gap-1 text-violet-500 dark:text-violet-400 hover:text-servlo-red dark:hover:text-servlo-red transition-colors"
  ...
  {#if open && hasWorktrees}
    <div
      class="absolute left-0 top-[calc(100%+4px)] z-20 min-w-[20rem] max-w-md rounded-sm border border-gray-200 dark:border-servlo-border bg-white dark:bg-servlo-bg shadow-lg overflow-hidden"
    >
      <div class="max-h-72 overflow-y-auto py-1">
        {#each entries as e (e.isMain ? '__main__' : e.branch)}
          {@const selected = e.isMain ? activeBranch === '' : e.branch === activeBranch}
          <div class="flex items-center gap-2 px-2 py-1.5 text-xs hover:bg-gray-50 dark:hover:bg-white/5 transition-colors {selected ? 'bg-servlo-red/5 dark:bg-servlo-red/10' : ''}">
            <button
              type="button"
              onclick={() => pick(e)}
              class="flex-1 flex items-center gap-2 min-w-0 text-left"
            >
              <svg class="w-3.5 h-3.5 shrink-0 {selected ? 'text-servlo-red' : 'text-violet-400'}" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                <path d="M6 3v12M15 6a3 3 0 1 0 6 0a3 3 0 1 0-6 0M3 18a3 3 0 1 0 6 0a3 3 0 1 0-6 0M18 9a9 9 0 0 1-9 9"/>
              </svg>
              <span class="font-mono truncate {selected ? 'text-servlo-red font-semibold' : 'text-gray-700 dark:text-gray-300'}">{e.branch}</span>
              {#if e.isMain}
            ...
            <button
              ...
              title={e.domain}
              class="shrink-0 text-gray-400 hover:text-servlo-red transition-colors p-1"
            >
