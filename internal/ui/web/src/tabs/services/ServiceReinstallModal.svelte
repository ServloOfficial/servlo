...
<Modal {open} {onclose} title={m.services_reinstall_title({ name: svc.name })} size="sm">
  <div class="px-5 py-4 space-y-3">
    ...
    <label class="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
      <input
        ...
        class="mt-0.5 w-4 h-4 rounded-sm border-gray-300 dark:border-servlo-border bg-white dark:bg-servlo-bg text-servlo-red focus:ring-servlo-red/40 cursor-pointer"
      />
      <span>
  ...
  {#snippet footer()}
    ...
    <button
      ...
      onclick={confirm}
      disabled={!canConfirm}
      class="text-xs px-3 py-1.5 rounded-sm {resetData ? 'bg-servlo-red hover:bg-servlo-redhov' : 'bg-servlo-red/80 hover:bg-servlo-red'} text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
