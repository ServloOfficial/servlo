...
describe('dsnFor', () => {
  it('rewrites the database path for a SQL DSN', () => {
    const dsn = dsnFor(engine('mysql://root:servlo@127.0.0.1:3306/servlo'), 'shop');
    expect(dsn).toBe('mysql://root:servlo@127.0.0.1:3306/shop');
  });

  it('keeps query params when swapping the mongo database', () => {
    const dsn = dsnFor(engine('mongodb://root:servlo@127.0.0.1:27017/?authSource=admin'), 'analytics');
    expect(dsn).toBe('mongodb://root:servlo@127.0.0.1:27017/analytics?authSource=admin');
  });
