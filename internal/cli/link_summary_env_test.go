...
// summary that preferred the backup reported the database servlo had just
...
func TestSummaryEnvReader_readsTheLiveEnvNotTheBackup(t *testing.T) {
	...
	writeFile(t, dir, ".env.before_servlo", "DB_CONNECTION=sqlite\nCACHE_STORE=database\n")

	read := summaryEnvReader(config.Site{Path: dir})
	if got := read("DB_CONNECTION"); got != "mysql" {
		t.Errorf("DB_CONNECTION = %q, want mysql (the live env, not the pre-servlo backup)", got)
	}
