...
func TestIsGhostContainerError(t *testing.T) {
	...
	// the run step. Captured from a user whose servlo-redis was stuck Created with
	...
	runErr := fmt.Errorf("podman run servlo-redis: %w",
	...
	cases := []struct {
		...
		{"container not known without store context", errors.New("no container with name servlo-redis: container not known"), false},
		{"port conflict", errors.New("rootlessport cannot expose privileged port 80, bind: address already in use"), false},
		{"missing image", errors.New(`short-name "servlo-php85-fpm:local" did not resolve to an alias`), false},
