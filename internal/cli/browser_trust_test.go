...
func TestBrowserTrustGuidance_atomic(t *testing.T) {
	...
	for _, want := range []string{"certutil", "rpm-ostree install nss-tools", "reboot"} {
...
func TestBrowserTrustGuidance_ordinary(t *testing.T) {
	...
	for _, want := range []string{"certutil", "nss-tools"} {
		if !strings.Contains(msg, want) {
...
func TestBrowserTrustStoreGuidanceNamesEveryStore(t *testing.T) {
	...
	for _, want := range []string{"/home/u/.pki/nssdb", "/home/u/.mozilla/firefox/p"} {
		if !strings.Contains(msg, want) {
