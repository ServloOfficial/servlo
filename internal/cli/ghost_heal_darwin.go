...
// retry once. The host podman client cannot purge the ghost: servlo's containers
...
func healGhostContainersIfNeeded(err error) bool {
	...
	purgeGhostServloContainers()
	return true
}

// purgeGhostServloContainers force-removes the stuck-Created servlo-* containers
...
func purgeGhostServloContainers() {
	...
	const script = `sudo podman ps -a --filter name=^servlo- --filter status=created -q | xargs -r sudo podman rm -f`
