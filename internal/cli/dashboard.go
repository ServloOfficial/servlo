...
func NewDashboardCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "dashboard",
		Short: "Open the Servlo dashboard (the desktop app if installed, else the browser)",
		Args:  cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			// Prefer the desktop app when it's the registered servlo:// handler;
			...
			if desktopnotify.AppInstalled() {
				if err := desktopnotify.OpenApp(""); err == nil {
					fmt.Println("Opening the Servlo desktop app")
					return nil
				}
			}
