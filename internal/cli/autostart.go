...
//
//  1. Strips the [Install] section from every servlo-*.container quadlet
...
// environment use `servlo stop` / `servlo start` — those are the live-state
// commands.
...
// quadletInstallBlock is the [Install] stanza appended to every servlo-*.container
...
// servlo-*.container quadlet on disk so its [Install] section is present
// (when enabled) or absent (when disabled), runs `systemctl --user
// daemon-reload`, then enables/disables (and starts/stops) every servlo-*
...
// duplicating the orchestration; the CLI goes through the same path.
func ApplyAutostart(disabled bool) error {
	...
	units := servloSystemd.AutostartUserUnits()
	for _, u := range units {
		name := strings.TrimSuffix(u, ".service")
		if disabled {
			_ = servloSystemd.DisableService(name)
		} else {
			_ = servloSystemd.EnableService(name)
		}
	}
	return nil
}

// rewriteQuadletsForAutostart walks every servlo-*.container in the user's
...
func rewriteQuadletsForAutostart(disabled bool) error {
	for _, path := range listServloQuadletFiles() {
...
// listServloQuadletFiles returns absolute paths of every servlo-*.container
// in the user's quadlet directory.
func listServloQuadletFiles() []string {
	entries, _ := filepath.Glob(filepath.Join(config.QuadletDir(), "servlo-*.container"))
	return entries
}

// listServloQuadlets returns the generated unit names (servlo-<name>) for
// every quadlet currently installed.
func listServloQuadlets() []string {
	files := listServloQuadletFiles()
