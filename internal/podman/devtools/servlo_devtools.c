...
#include "php_servlo_devtools.h"

ZEND_DECLARE_MODULE_GLOBALS(servlo_devtools)

PHP_INI_BEGIN()
	STD_PHP_INI_ENTRY("servlo.devtools_host", "", PHP_INI_ALL, OnUpdateString, host, zend_servlo_devtools_globals, servlo_devtools_globals)
	STD_PHP_INI_ENTRY("servlo.devtools_kinds", "query", PHP_INI_ALL, OnUpdateString, kinds, zend_servlo_devtools_globals, servlo_devtools_globals)
	STD_PHP_INI_ENTRY("servlo.devtools_flag", "/usr/local/etc/servlo/devtools.flag", PHP_INI_ALL, OnUpdateString, flag, zend_servlo_devtools_globals, servlo_devtools_globals)
PHP_INI_END()
...
#define SERVLO_OBSERVE 1
#endif

#ifdef SERVLO_OBSERVE
...
/* servlo_send writes one NDJSON line to the configured socket and closes it.
 * Fire-and-forget with a 50ms connect budget; any error is swallowed so a
 * down or slow servlo-panel never disturbs the request. */
static void servlo_send(const char *line, size_t len)
{
	const char *host = SERVLO_G(host);
	if (!host || !host[0]) {
		return;
	}
 ...
 * inspecting. servlo_is_worker is set once at MINIT from the process command
 * line so the observer can skip them. Detected here rather than via a
 * servlo-set env var so it works without re-launching workers with new flags. */
static zend_bool servlo_is_worker = 0;

/* servlo_laravel is set once the Laravel adapter has been loaded at boot. While
...
static zend_bool servlo_laravel = 0;
...
/* servlo_worker_cmd holds this process's artisan command (e.g. "queue:work",
 * "scrape:rtb-data") so worker queries can be labelled and filtered by it. */
static char servlo_worker_cmd[64] = "";
static void extract_self_cmd(void)
{
	...
	while (k < n) {
		...
		if (strcmp(base, "artisan") == 0) {
			...
			if (next < n && buf[next] != '-') {
				strncpy(servlo_worker_cmd, &buf[next], sizeof(servlo_worker_cmd) - 1);
				servlo_worker_cmd[sizeof(servlo_worker_cmd) - 1] = '\0';
			}
			return;
		}
		k += (ssize_t)len + 1;
...
static void detect_worker(void)
{
	...
	for (int depth = 0; depth < 6 && pid > 1; depth++) {
		if (cmdline_is_worker(pid)) {
			servlo_is_worker = 1;
			extract_self_cmd();
			if (!servlo_worker_cmd[0]) {
				strcpy(servlo_worker_cmd, "worker");
			}
			return;
		}
		pid = read_ppid(pid);
...
static zend_bool servlo_in_test(void)
{
	if (SERVLO_G(is_test)) {
		return 1;
	}
...
static unsigned long servlo_seq = 0;
static void append_event_id(smart_str *buf)
{
	...
	char out[40];
	snprintf(out, sizeof(out), "%015llx%08lx", us, (servlo_seq++ & 0xffffffffUL));
	smart_str_appends(buf, out);
}
...
{
	if (SERVLO_G(tstack_len) == SERVLO_G(tstack_cap)) {
		int cap = SERVLO_G(tstack_cap) ? SERVLO_G(tstack_cap) * 2 : 16;
		double *p = realloc(SERVLO_G(tstack), (size_t)cap * sizeof(double));
		if (!p) {
			return;
		}
		SERVLO_G(tstack) = p;
		SERVLO_G(tstack_cap) = cap;
	}
	SERVLO_G(tstack)[SERVLO_G(tstack_len)++] = v;
}

static double tstack_pop(void)
{
	if (SERVLO_G(tstack_len) <= 0) {
		return -1.0;
	}
	return SERVLO_G(tstack)[--SERVLO_G(tstack_len)];
}
...
static void servlo_obs_begin(zend_execute_data *execute_data)
{
	tstack_push(now_ms());
}

static void servlo_obs_end(zend_execute_data *execute_data, zval *retval)
{
	...
	zval bound;
	ZVAL_UNDEF(&bound);
	if (is_stmt_execute && SERVLO_G(bind_buf_init) && Z_TYPE(execute_data->This) == IS_OBJECT) {
		zend_ulong h = (zend_ulong) Z_OBJ_HANDLE(execute_data->This);
		zval *b = zend_hash_index_find(&SERVLO_G(bind_buf), h);
		if (b) {
			ZVAL_COPY(&bound, b);
			zend_hash_index_del(&SERVLO_G(bind_buf), h);
		}
	}

	if (!SERVLO_G(active) || !SERVLO_G(want_query) || start < 0 ||
		servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		zval_ptr_dtor(&bound);
		return;
	}
	...
	smart_str sctx = {0};
	const char *site = server_var("SERVLO_SITE", sizeof("SERVLO_SITE") - 1, &sctx);
	smart_str sbranch = {0};
	const char *branch = server_var("SERVLO_BRANCH", sizeof("SERVLO_BRANCH") - 1, &sbranch);
	...
	if (SERVLO_G(rid)[0]) {
		smart_str_appendc(&buf, ',');
		append_json_kv_str(&buf, "rid", SERVLO_G(rid), strlen(SERVLO_G(rid)));
	}
	if (servlo_is_worker && servlo_worker_cmd[0]) {
		smart_str_appendc(&buf, ',');
		append_json_kv_str(&buf, "worker", servlo_worker_cmd, strlen(servlo_worker_cmd));
	}
	if (servlo_in_test()) {
	...
	servlo_send(ZSTR_VAL(buf.s), ZSTR_LEN(buf.s));
...
/* servlo_bind_end buffers PDOStatement::bindValue/bindParam values per statement
...
static void servlo_bind_end(zend_execute_data *execute_data, zval *retval)
{
	if (!SERVLO_G(active) || !SERVLO_G(want_query) || servlo_laravel) {
		return;
	}
	...
	if (!SERVLO_G(bind_buf_init)) {
		zend_hash_init(&SERVLO_G(bind_buf), 8, NULL, ZVAL_PTR_DTOR, 0);
		SERVLO_G(bind_buf_init) = 1;
	}
	zend_ulong h = (zend_ulong) Z_OBJ_HANDLE(execute_data->This);
	zval *arr = zend_hash_index_find(&SERVLO_G(bind_buf), h);
	if (!arr) {
		zval newarr;
		array_init(&newarr);
		arr = zend_hash_index_add(&SERVLO_G(bind_buf), h, &newarr);
	}
	zval copy;
...
/* servlo_boot_end fires when Illuminate\Foundation\Application::boot() returns,
 ...
 * fataling. */
static void servlo_boot_end(zend_execute_data *execute_data, zval *retval)
{
	(void)execute_data;
	(void)retval;
	if (servlo_laravel || !SERVLO_G(active) || !SERVLO_G(want_query)) {
		return;
	}
	servlo_laravel = 1; /* set first: stops the PDO observer double-capturing */
	zend_eval_string(
		"if (function_exists('app') && !defined('SERVLO_LARAVEL_ADAPTER')) {"
		" define('SERVLO_LARAVEL_ADAPTER', 1);"
		" @include '/usr/local/etc/servlo/laravel-adapter.php';"
		"}",
		NULL, "servlo-laravel-adapter");
}
...
static void servlo_ensure_collector(void)
{
	if (SERVLO_G(collector_loaded)) {
		return;
	}
	SERVLO_G(collector_loaded) = 1;
	zend_eval_string(
		"if (!function_exists('Servlo\\\\Collector\\\\emit')) {"
		" @include '/usr/local/etc/servlo/devtools-collector.php';"
		"}",
		NULL, "servlo-collector-load");
}

/* servlo_mail_end captures one outgoing mail. Laravel claims mail via its own
...
static void servlo_mail_end(zend_execute_data *execute_data, zval *retval)
{
	(void)retval;
	if (!SERVLO_G(active) || servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		return;
	}
	if (ZEND_CALL_NUM_ARGS(execute_data) < 1) {
		return;
	...
	servlo_ensure_collector();
	zval fname, rv, args[1];
	ZVAL_STRINGL(&fname, "Servlo\\Collector\\mail", sizeof("Servlo\\Collector\\mail") - 1);
...
/* servlo_view_end captures one Twig render. Twig is the de-facto Symfony view
...
static void servlo_view_end(zend_execute_data *execute_data, zval *retval)
{
	(void)retval;
	if (!SERVLO_G(active) || servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		return;
	...
	ZVAL_STRINGL(&fname, "Servlo\\Collector\\view", sizeof("Servlo\\Collector\\view") - 1);
...
/* servlo_event_end captures one Symfony event dispatch. The dispatcher is the
...
static void servlo_event_end(zend_execute_data *execute_data, zval *retval)
{
	if (!SERVLO_G(active) || servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		return;
	}
	...
	ZVAL_STRINGL(&fname, "Servlo\\Collector\\event", sizeof("Servlo\\Collector\\event") - 1);
...
/* servlo_job_end captures one message dispatched to the Symfony Messenger bus.
...
static void servlo_job_end(zend_execute_data *execute_data, zval *retval)
{
	(void)retval;
	if (!SERVLO_G(active) || servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		return;
	...
	servlo_ensure_collector();
	zval fname, rv, args[1];
	ZVAL_STRINGL(&fname, "Servlo\\Collector\\job", sizeof("Servlo\\Collector\\job") - 1);
	ZVAL_COPY(&args[0], msg);
	if (call_user_function(NULL, NULL, &fname, &rv, 1, args) == SUCCESS) {
		zval_ptr_dtor(&rv);
	}
...
/* servlo_http_begin captures one outgoing Symfony HttpClient request. We hook the
...
static void servlo_http_begin(zend_execute_data *execute_data)
{
	if (!SERVLO_G(active) || servlo_laravel || (servlo_is_worker && !SERVLO_G(capture_workers))) {
		return;
	}
	...
	servlo_ensure_collector();
	zval fname, rv, args[2];
	ZVAL_STRINGL(&fname, "Servlo\\Collector\\http", sizeof("Servlo\\Collector\\http") - 1);
...
static zend_observer_fcall_handlers servlo_observer_init(zend_execute_data *execute_data)
{
	...
	if (match) {
		h.begin = servlo_obs_begin;
		h.end = servlo_obs_end;
	}
	return h;
}
#endif /* SERVLO_OBSERVE */

/* ── module lifecycle ───────────────────────────────────────────────────── */

static PHP_GINIT_FUNCTION(servlo_devtools)
{
#if defined(COMPILE_DL_SERVLO_DEVTOOLS) && defined(ZTS)
	ZEND_TSRMLS_CACHE_UPDATE();
#endif
	memset(servlo_devtools_globals, 0, sizeof(*servlo_devtools_globals));
}

PHP_MINIT_FUNCTION(servlo_devtools)
{
	REGISTER_INI_ENTRIES();
#ifdef SERVLO_OBSERVE
	detect_worker();
	zend_observer_fcall_register(servlo_observer_init);
#endif
	return SUCCESS;
}

PHP_MSHUTDOWN_FUNCTION(servlo_devtools)
{
	UNREGISTER_INI_ENTRIES();
	return SUCCESS;
}

PHP_RINIT_FUNCTION(servlo_devtools)
{
#if defined(ZTS) && defined(COMPILE_DL_SERVLO_DEVTOOLS)
	ZEND_TSRMLS_CACHE_UPDATE();
#endif
#ifdef SERVLO_OBSERVE
	SERVLO_G(tstack_len) = 0;
	SERVLO_G(collector_loaded) = 0;
	SERVLO_G(bind_buf_init) = 0;
	SERVLO_G(is_test) = 0;
	...
	servlo_laravel = 0;
	SERVLO_G(active) = (SERVLO_G(flag) && SERVLO_G(flag)[0] && access(SERVLO_G(flag), F_OK) == 0) ? 1 : 0;
	SERVLO_G(want_query) = (SERVLO_G(kinds) && strstr(SERVLO_G(kinds), "query")) ? 1 : 0;
	SERVLO_G(capture_workers) = (access("/usr/local/etc/servlo/devtools-workers.flag", F_OK) == 0) ? 1 : 0;
	 ...
	 * unique without needing an RNG. */
	{
		...
		snprintf(SERVLO_G(rid), sizeof(SERVLO_G(rid)), "%015llx%lx%lx", us, (unsigned long)getpid(), (servlo_seq++ & 0xffffUL));
	}
	...
	{
		zend_bool on = SERVLO_G(active) && SERVLO_G(want_query) && (!servlo_is_worker || SERVLO_G(capture_workers));
		zend_register_bool_constant("SERVLO_DEVTOOLS_ON", sizeof("SERVLO_DEVTOOLS_ON") - 1, on, 0, module_number);
		zend_register_string_constant("SERVLO_DEVTOOLS_WORKER", sizeof("SERVLO_DEVTOOLS_WORKER") - 1, servlo_worker_cmd, 0, module_number);
		/* The agnostic collector groups its events with this request's queries. */
		zend_register_string_constant("SERVLO_DEVTOOLS_RID", sizeof("SERVLO_DEVTOOLS_RID") - 1, SERVLO_G(rid), 0, module_number);
	}
#endif
	return SUCCESS;
}

PHP_RSHUTDOWN_FUNCTION(servlo_devtools)
{
#ifdef SERVLO_OBSERVE
	if (SERVLO_G(bind_buf_init)) {
		zend_hash_destroy(&SERVLO_G(bind_buf));
		SERVLO_G(bind_buf_init) = 0;
	}
#endif
	return SUCCESS;
...
PHP_GSHUTDOWN_FUNCTION(servlo_devtools)
{
#ifdef SERVLO_OBSERVE
	if (servlo_devtools_globals->tstack) {
		free(servlo_devtools_globals->tstack);
		servlo_devtools_globals->tstack = NULL;
	}
#endif
}

PHP_MINFO_FUNCTION(servlo_devtools)
{
	php_info_print_table_start();
	php_info_print_table_header(2, "servlo_devtools support", "enabled");
	php_info_print_table_row(2, "version", PHP_SERVLO_DEVTOOLS_VERSION);
#ifdef SERVLO_OBSERVE
...
zend_module_entry servlo_devtools_module_entry = {
	STANDARD_MODULE_HEADER,
	"servlo_devtools",
	NULL, /* functions */
	PHP_MINIT(servlo_devtools),
	PHP_MSHUTDOWN(servlo_devtools),
	PHP_RINIT(servlo_devtools),
	PHP_RSHUTDOWN(servlo_devtools),
	PHP_MINFO(servlo_devtools),
	PHP_SERVLO_DEVTOOLS_VERSION,
	PHP_MODULE_GLOBALS(servlo_devtools),
	PHP_GINIT(servlo_devtools),
	PHP_GSHUTDOWN(servlo_devtools),
	NULL,
	STANDARD_MODULE_PROPERTIES_EX
};

#ifdef COMPILE_DL_SERVLO_DEVTOOLS
#ifdef ZTS
ZEND_TSRMLS_CACHE_DEFINE()
#endif
ZEND_GET_MODULE(servlo_devtools)
#endif
