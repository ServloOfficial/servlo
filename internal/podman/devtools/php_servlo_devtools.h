/* servlo_devtools — engine-level capture for the servlo debug window.
 *
 ...
 * JSON event to the same Unix/TCP socket the debug bridge uses, where servlo-panel
 ...
 */
#ifndef PHP_SERVLO_DEVTOOLS_H
#define PHP_SERVLO_DEVTOOLS_H

extern zend_module_entry servlo_devtools_module_entry;
#define phpext_servlo_devtools_ptr &servlo_devtools_module_entry

#define PHP_SERVLO_DEVTOOLS_VERSION "0.1.0"

#include "php.h"

ZEND_BEGIN_MODULE_GLOBALS(servlo_devtools)
	char *host;       /* servlo.devtools_host: unix:///… or tcp://host:port */
	char *kinds;      /* servlo.devtools_kinds: comma list, e.g. "query"    */
	char *flag;       /* servlo.devtools_flag: sentinel path, stat per req  */
...
ZEND_END_MODULE_GLOBALS(servlo_devtools)

#if defined(ZTS) && defined(COMPILE_DL_SERVLO_DEVTOOLS)
ZEND_TSRMLS_CACHE_EXTERN()
#endif

#define SERVLO_G(v) ZEND_MODULE_GLOBALS_ACCESSOR(servlo_devtools, v)

#endif /* PHP_SERVLO_DEVTOOLS_H */
