...
import (
	...
	"github.com/ServloOfficial/servlo/internal/php"
...
func phpVersionTestSite(t *testing.T, tweak func(*config.Site)) *config.Site {
	t.Helper()
	tmp := t.TempDir()
	// HOME too, not just XDG: the systemd unit for the fpm service is written
	// under os.UserHomeDir(), which reads $HOME rather than the XDG dirs.
	// Without this the version switch
