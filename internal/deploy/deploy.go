...
func Run(o Options) (Result, error) {
	...
	if hasCommands(script) {
		...
		if err := o.RunScript(o.Site.Path, script, o.Out); err != nil {
			...
			// the half-deployed code. Deploy is a pull in place (CLAUDE.md
			// §3.5) and the tree is not rolled back, so this is a real limit
			// rather than a bug to fix here: on a busy site the cache is warm
			// and it holds, on a quiet one it may not.
